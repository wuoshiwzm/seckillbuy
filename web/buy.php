...
//获取redis对象，查询活动和商品信息，不查mysql
$redis_obj = \common\Datasource::getRedis('instance1');
$d_list = array(
    'u_trade_' . $uid . '_' . $active_id,  //订单信息
    'st_a_' . $active_id //活动状态信息
);
/**
 * id, sys_status,
 * num_user, num_left,
...
foreach ($goods as $i => $goods_id) {
    $d_list[] = 'info_g_' . $goods_id;  // 商品详情  哈希表
}
...
foreach ($data_list as $i => $goods_info) {
    $goods_num = $nums[$i - 2];//unset 了 $data_list[0], $data_list[1]
//    $goods_info = $goods_model->get($goods_id);  这里已经从redis的$data_list 中读到了
...
// 8 扣除商品剩余数量
    ...
    //更新数据库之前更新缓存
    $left = $goods_model->changeLeftNumCached($goods_id, 0-$goods_num);//用hioncrby 更新，避免并发写错数据
    $ok = false;
    //更新数据库
    if ($left >= 0) {
